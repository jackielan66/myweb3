...
const ERC20TokenClaim: React.FC<{ poolId: string }> = (props) => {
  const { poolId } = props
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      setLoading(false)
      useMapInfo.refetch()
      rccTokenAddressBalance.refetch()
        ...
        <Container>
          ...
          <Box sx={{
            ...
            <Typography variant='subtitle1' >
              质押的{stTokenBaseInfo.name}数量:{useMapInfo.data && formatEther(useMapInfo.data[0] || '')} {stTokenBaseInfo.symbol}
            </Typography>
            <Typography variant='subtitle1' >
...
export default ERC20TokenClaim;
